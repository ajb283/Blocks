...
class Model {
    ...
    Model(Model model) {
        ...
        _hand = new ArrayList<>(model._hand);
        _score = model.score();
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int x = (nrows * _width + ncols * _height);
        return (x - nrows * ncols) + (x * _streakLength);
    }

    /** Return true iff the current hand is empty (i.e., piece(k) is null
     *  for all k). */
    boolean handUsed() {
        if (_hand == null || _hand.size() == 0) {
            return true;
        }
        for (Piece p : _hand) {
            if (p != null) {
    ...
    void undo() {
        if (_current > 0) {
            _current -= 1;
            GameState state = _history.get(_current);
            deepCopy(state._savedCells, _cells);
            _hand = new ArrayList<>(state._savedHand);
            _score = state._savedScore;
            _streakLength = state._savedStreakLength;
        }
    }
